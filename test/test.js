describe("EasyChain", function () {
    ...
    it("should not run functions that doesn't invoke the given callback.", function () {
        ...
        chain3
            .all(function () {}, function (next) { next() }, function (next) { next() })
            .single(function () {
                spiedFunction();
            })
            .run();
    ...
    it("should run functions that returns a jQuery promise.", function () {
        var deferredObjects = _(new Array(8)).map(function () { return $.Deferred(); })
    ...
    it("should fire progress callback.", function () {
        var callback = function () {
            var args = arguments;
            return function (next) { next.apply({}, args) };
        };
        ...
        chain
            ...
            .on(EasyChain.PROGRESS, function (event) {
                ...
                if (event.index === 0) {
                    ...
                    expect(event.values).toEqual([["H", "E"], undefined, undefined]);
                }
                spiedFunction();
            })
            .run();
    ...
    it("should trigger error event when timeout occurs.", function (done) {
        chain
            .all(function (next) { next() })
            .all(function (next) { next("H", "I") }, function (next) { })
